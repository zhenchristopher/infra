...
resource "google_compute_region_instance_group_manager" "pool" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = var.capacity_manager_max_size >= 1 && var.capacity_manager_max_size <= 10 && floor(var.capacity_manager_max_size) == var.capacity_manager_max_size
      error_message = "Client capacity manager max size must be an integer between 1 and 10; the root fixes the environment ceiling."
    }
